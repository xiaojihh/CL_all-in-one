...
path='/data/jyl/datasets'

ots_traindata=RESIDE_Dataset(path+'/RESIDE/OTS_beta',train=True,format='.jpg')
...
snow100k_traindata = Snow100K_Dataset(path+'/Snow100K/train', train=True)
snow100k_testdata = Snow100K_Dataset(path+'/Snow100K/test/Snow100K-M', train = False, size='whole img')

ots_loadertrain=DataLoader(dataset=ots_traindata, batch_size=opt.bs, shuffle=True)
ots_loadertest=DataLoader(dataset=ots_testdata, batch_size=1, shuffle=True)
rain100h_loadertrain=DataLoader(dataset=rain100h_traindata, batch_size=opt.bs, shuffle=True)
rain100h_loadertest=DataLoader(dataset=rain100h_testdata, batch_size=1, shuffle=True)
snow100k_loadertrain = DataLoader(dataset=snow100k_traindata, batch_size=opt.bs, shuffle=True)
